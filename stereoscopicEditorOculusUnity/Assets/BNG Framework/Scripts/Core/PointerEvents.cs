...
namespace BNG
{
    public class PointerEvents : MonoBehaviour, IPointerClickHandler, IPointerEnterHandler, IPointerExitHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        void Start()
        {
            ...
            // Register the OnClickEvent to automatically set the selected object
            if (selectedObjectTracker != null)
            {
                OnPointerClickEvent.AddListener((eventData) => { selectedObjectTracker.UpdateSelectedObject(gameObject); });
            }
            else
            {
                Debug.LogWarning("No SelectedObjectTracker found in the scene.");
            }
        }
